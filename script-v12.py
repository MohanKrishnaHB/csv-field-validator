...
def isNotCSVOrGz(fileName):
    if (len(os.path.splitext(fileName)) > 1) and (len(os.path.splitext(fileName)[1]) > 1) and not (os.path.splitext(fileName)[1].lower() == '.csv' or os.path.splitext(fileName)[1].lower() == '.gz' or os.path.splitext(fileName)[1].lower() == '.zip'):
        return True
    return False
def get_non_csv_files(folder_path):
    try:
        files = os.listdir(folder_path)
        return [item for item in files if item is not None and isNotCSVOrGz(item)]
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return []
...
def get_dates(file_name, file_type):
    global errors
    try:
        target = pd.read_csv(file_name, usecols=[file_type['Date Field']], dtype={file_type['Date Field']: str}, low_memory=False)
...
def check_count_against_date(file_path, date_column, target_date, columns_to_check):
    ...
    chunks = pd.read_csv(file_path, chunksize=chunk_size, parse_dates=[date_column], low_memory=False)
...
def process_files(folder, date_to_append, master_data, date_to_validate, debug):
    ...
    for file in tqdm(files, desc="Processing files", unit="file"):
        ...
        if file_type:
            ...
            if predefined_columns:
                ...
                #     count_check_output = check_count_against_date(file_path, file_type['DateField to check count for'], date_to_validate_count, list(map((lambda item: item.strip()), file_type['Count Field For Date'].split(','))))
    ...
    nonCsvOrGzFiles = get_non_csv_files(folder)
    if nonCsvOrGzFiles is None:
        nonCsvOrGzFiles = []
    nonCsvOrGzFilesError = [({
        'Error_File': item,
        'Missing_Columns': 'NA',
        'date_missing': 'NA',
        'count_missing': 'NA',
